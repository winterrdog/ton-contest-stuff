...
requires std::invocable<FunctionType> &&std::is_same_v<void, std::invoke_result_t<FunctionType>> class thread_pool
{
  public:
    ...
    requires std::invocable<InitializationFunction, std::size_t>
        &&std::is_same_v<void, std::invoke_result_t<InitializationFunction, std::size_t>> explicit thread_pool(
     ...
     * @return A std::future<ReturnType> that can be used to retrieve the returned
     * value.
     */
    template <typename Function, typename... Args, typename ReturnType = std::invoke_result_t<Function &&, Args &&...>>
    requires std::invocable<Function, Args...> [[nodiscard]] std::future<ReturnType> enqueue(Function f, Args... args)
    {
#ifdef __cpp_lib_move_only_function
        // we can do this in C++23 because we now have support for move only
        // functions
    ...
    /**
     * @brief Enqueue a task to be executed in the thread pool. Any return value
    ...
    requires std::invocable<Function, Args...> void enqueue_detach(Function &&func, Args &&...args)
    {
     ...
     * @details Note that this does not guarantee that all tasks will be cleared,
     * as currently running tasks could add additional tasks. Also a thread could
     * steal a task from another in the middle of this.
